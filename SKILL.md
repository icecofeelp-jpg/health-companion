...
## ⚠️ 日期判断：必须用实际系统日期，不能靠"最新文件"

Cron任务的prompt中**绝对不能**写"今天是几号，你自己去目录下看最新的文件是什么日期"。这个逻辑在文件创建延迟时会出错——比如早上cron没创建文件，晚上cron就读到前一天的旧文件，导致报告日期错一天。

**正确做法**：prompt开头直接写"今天是2026年5月25日（如果实际日期不同，请用实际日期）"，然后写"今天的记录在 ~/.hermes/health-records/2026-05-25.md。如果这个文件不存在，先用模板创建它"。

cron运行时会根据实际日期生成对应文件名的路径，AI自己会根据日期自动替换文件名（如5月26日就替换成"2026-05-26.md"），无需手动干预。这个格式AI能正确理解并自动适配。

**错误做法**（已废弃）：prompt里写"今天是几号，你自己去目录下看最新的文件"。

---

### 查看历史记录
查看 `~/.hermes/health-records/` 目录下的 md 文件。
